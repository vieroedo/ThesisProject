...
transfer_body_order = ['Jupiter', 'Europa', 'Jupiter', 'Callisto', 'Jupiter']
...
transfer_leg_settings, transfer_node_settings = transfer_trajectory.mga_transfer_settings(
    transfer_body_order,
    leg_type,
    minimum_pericenters = {'Jupiter': 600000000.0,'Io': 2000e3, 'Europa': 1900e3, 'Ganymede': 2900e3, 'Callisto': 2750e3},
...
default_trajectory_parameters = [5.6,# * Entry 0: Arrival velocity in km/s
                                 11000.,# * Entry 1: Initial epoch in Julian days since J2000
                                 50.,# * Entry 2: Period of Target Orbit in Julian days
                                 30.,# * Entry 3: Time-of-flight from the arrival at Jupiter to the flyby at the first moon in Julian days
                                 4.,# * Entry 4: Time-of-flight from the flyby at the first moon to the atmospheric entry at Jupiter in Julian days
...
node_times = list()
node_times.append( ( default_trajectory_parameters[1] ) * julian_day )
node_times.append( node_times[ 0 ] + default_trajectory_parameters[3] * julian_day )
node_times.append( node_times[ 1 ] + default_trajectory_parameters[4] * julian_day )
node_times.append( node_times[ 2 ] + default_trajectory_parameters[5] * julian_day )
node_times.append( node_times[ 3 ] + 25 * julian_day )
...
node_free_parameters.append( np.array( [ 20000 + 6e8, 1.6, 22000.0 ] ) )
